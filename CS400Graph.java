...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        PriorityQueue<CS400Graph<T>.Path> pathes = new PriorityQueue<Path>(); 
        Vertex stvert =vertices.get(start);
        Vertex endvert =vertices.get(end);
        if (stvert==null||endvert==null )
          throw new NoSuchElementException("no vertex");
        Path begin= new Path(stvert);
        for(int x = 0; x<stvert.edgesLeaving.size();x++) {
          pathes.add(new Path(begin, stvert.edgesLeaving.get(x)));
        }
        while (!pathes.isEmpty()) {
          if(pathes.peek().end.equals(endvert))
            return pathes.peek();
          for(int x = 0; x<pathes.peek().end.edgesLeaving.size();x++) {
            pathes.add(new Path(pathes.peek(), pathes.peek().end.edgesLeaving.get(x)));
          }
          pathes.poll();
        }
